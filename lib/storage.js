(function () {
  ...
  function normalizeRole(row, index = 0) {
    const title = String(field(row, ["title", "岗位名称", "岗位", "role"], "未命名岗位")).trim();
    const location = String(field(row, ["location", "国家/城市", "地点", "城市"], "未填写地点")).trim();
    const unit = String(field(row, ["businessUnit", "事业部/产品线", "事业部", "产品线"], "不限产品")).trim();
    const rawOpenCount = field(row, ["openCount", "HC", "hc", "headcount"], 1);
    return {
      id: String(field(row, ["id"], stableId(`${title}|${location}|${unit}|${index}`))),
      priority: String(field(row, ["priority", "优先级"], "S")).toUpperCase(),
      businessUnit: unit,
      function: String(field(row, ["function", "职能", "functionName"], "General")),
      region: String(field(row, ["region", "大区"], "全球")),
      title,
      location,
      nationality: String(field(row, ["nationality", "国籍要求", "国籍"], "")),
      openCount: Number.isFinite(Number(rawOpenCount)) ? Math.max(0, Number(rawOpenCount)) : 1,
      note: String(field(row, ["note", "备注", "JD", "jd"], "")),
      hiringManager: String(field(row, ["hiringManager", "HM", "招聘负责人"], "")),
      updatedAt: String(field(row, ["updatedAt", "更新日期", "更新时间"], new Date().toISOString().slice(0, 10))),
    };
  }
  ...
  globalThis.SanyStore = { CURRENT_YEAR, KEY, normalizeRole, normalizeRoles, getHcs, saveHcs, replaceImportedRows, getHistory, appendHistory, clearHistory, getAgentMessages, saveAgentMessages };
})();
