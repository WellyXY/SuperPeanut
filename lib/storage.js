(function () {
  ...
  const KEY = {
    workspace: "superpeanut_workspace_key",
    session: "superpeanut_account_session",
    account: "superpeanut_account_user",
  };
  let cachedState = null;
  let loadingState = null;
  let cachedSessionToken = "";
  let loadingSessionToken = "";
  ...
  const normalizeLocation = (value) => {
    ...
    return location
      .replace(/[，,]\s*(?:中国大陆|中國大陸)\s*$/u, "")
      .replace(/[（(][^）)]*(?:华为|華為|任命|国代|國代|名额|名額|人选|人選)[^）)]*[）)]\s*$/u, "")
      .trim() || location;
  };
  ...
  function normalizeRole(row, index = 0) {
    ...
    const location = normalizeLocation(field(row, ["location", "国家/城市", "地点", "城市"], "未填写地点"));
    const company = normalizeCompany(field(row, ["company", "Company", "公司", "公司名称", "雇主", "Employer"], ""));
  ...
  function normalizeSkills(rows) {
    ...
    })).filter((skill) => skill.company && skill.name && skill.description && skill.content);
  }
  ...
  const compactComparable = (value) => cleanText(value).toLowerCase().replace(/[\s\p{P}\p{S}]+/gu, "");

  function likelySameImportedRole(left, right) {
    if (normalizeCompany(left?.company).toLowerCase() !== normalizeCompany(right?.company).toLowerCase()) return false;
    if (compactComparable(left?.title) !== compactComparable(right?.title)) return false;
    const leftLocation = compactComparable(left?.location);
    const rightLocation = compactComparable(right?.location);
    if (leftLocation && leftLocation === rightLocation) return true;
    if (!leftLocation || !rightLocation || (!leftLocation.includes(rightLocation) && !rightLocation.includes(leftLocation))) return false;
    const leftNote = compactComparable(left?.note);
    const rightNote = compactComparable(right?.note);
    if (Math.min(leftNote.length, rightNote.length) < 40) return false;
    return leftNote.includes(rightNote) || rightNote.includes(leftNote);
  }
  ...
  async function request(path, body) {
    const stored = await chrome.storage.local.get([KEY.session, KEY.account]);
    const token = String(stored[KEY.session] || "");
    if (!token) throw new Error("请点击 SuperPeanut 插件图标登录账号");
    cachedSessionToken = token;
    const response = await fetch(`${STORAGE_ENDPOINT}${path}`, {
      method: "POST",
      headers: { "content-type": "application/json", "x-superpeanut-session": token },
      body: body ? JSON.stringify(body) : undefined,
    });
    const payload = await response.json().catch(() => ({}));
    if (response.status === 401) {
      await chrome.storage.local.remove([KEY.session, KEY.account]);
      cachedState = null;
    }
  ...
  async function login(username, password) {
    const response = await fetch(`${STORAGE_ENDPOINT}/v1/auth/login`, {
      method: "POST",
    ...
    await chrome.storage.local.set({ [KEY.session]: payload.token, [KEY.account]: payload.user });
    cachedState = null;
  ...
  async function register(displayName, username, password) {
    const response = await fetch(`${STORAGE_ENDPOINT}/v1/auth/register`, {
      method: "POST",
      headers: { "content-type": "application/json" },
  ...
  async function logout() {
    const stored = await chrome.storage.local.get(KEY.session);
    const token = String(stored[KEY.session] || "");
    if (token) await fetch(`${STORAGE_ENDPOINT}/v1/auth/logout`, { method: "POST", headers: { "x-superpeanut-session": token } }).catch(() => null);
    await chrome.storage.local.remove([KEY.session, KEY.account]);
    cachedState = null;
    cachedSessionToken = "";
  }
  ...
  async function currentAccount() {
    const stored = await chrome.storage.local.get([KEY.session, KEY.account]);
    return stored[KEY.session] && stored[KEY.account] ? stored[KEY.account] : null;
  }

  async function loadState(force = false) {
    const stored = await chrome.storage.local.get(KEY.session);
    const activeToken = String(stored[KEY.session] || "");
    if (activeToken !== cachedSessionToken) {
      cachedState = null;
      loadingState = null;
      cachedSessionToken = activeToken;
    }
    if (cachedState && !force) return cachedState;
    if (!loadingState) {
      loadingSessionToken = activeToken;
      loadingState = request("/v1/state/read").then((payload) => {
        const state = { hcs: normalizeRoles(payload.state?.hcs), history: Array.isArray(payload.state?.history) ? payload.state.history : [], messages: Array.isArray(payload.state?.messages) ? payload.state.messages : [], skills: normalizeSkills(payload.state?.skills) };
        if (cachedSessionToken === activeToken) cachedState = state;
        return state;
      }).finally(() => {
        if (loadingSessionToken === activeToken) loadingState = null;
      });
    }
    return loadingState;
  }
  ...
  async function saveState(partial) {
    ...
    const next = {
      ...
      skills: partial.skills === undefined ? current.skills : normalizeSkills(partial.skills),
    };
    const payload = await request("/v1/state/write", { state: next });
    cachedState = { hcs: normalizeRoles(payload.state?.hcs), history: Array.isArray(payload.state?.history) ? payload.state.history : [], messages: Array.isArray(payload.state?.messages) ? payload.state.messages : [], skills: normalizeSkills(payload.state?.skills) };
    return cachedState;
  }
  ...
  async function mergeImportedRows(rows) {
    ...
    for (const role of incoming) {
      ...
      const existingIndex = merged.findIndex((existing) =>
        ...
        likelySameImportedRole(existing, role)
      );
      if (existingIndex >= 0) {
  ...
  async function getCompanySkills() { return (await loadState()).skills; }
  async function saveCompanySkills(skills) { return (await saveState({ skills })).skills; }

  globalThis.SanyStore = { CURRENT_YEAR, KEY, login, register, logout, currentAccount, normalizeCompany, normalizeLocation, normalizeRole, normalizeRoles, normalizeSkills, getHcs, saveHcs, replaceImportedRows, mergeImportedRows, getHistory, appendHistory, clearHistory, getAgentMessages, saveAgentMessages, getCompanySkills, saveCompanySkills };
})();
