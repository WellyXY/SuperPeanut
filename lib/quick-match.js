(function () {
  ...
  const PRODUCT_GROUPS = [
    ...
    { id: "road", terms: ["road machinery", "paving", "paver", "compactor", "roller", "asphalt", "路机", "摊铺", "压路机", "沥青"] },
  ...
  function productFit(candidateText, roleText) {
    ...
    if (!roleGroups.length || containsAny(roleText, ["不限产品", "all products", "full line"])) {
  ...
  function matchCandidateToRoles(candidate, roles) {
    ...
    for (const role of Array.isArray(roles) ? roles : []) {
      ...
      const roleFunctions = functionGroupsFor(roleText);
      const domainSensitive = roleFunctions.some((group) => ["sales", "service", "technical", "product"].includes(group));
      if (!functional || (domainSensitive && product < 0.45)) continue;
